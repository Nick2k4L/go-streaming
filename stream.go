...
func logConnectionEstablished(conn *StreamConn) {
	log.WithFields(map[string]interface{}{
		...
		"negotiatedMTU": conn.getNegotiatedMTU(),
		"rtt":           conn.rtt,
...
func (s *StreamConn) logIgnoredPacket(pkt *Packet) {
	log.WithField("flags", pkt.Flags).Debug("received non-SYN-ACK packet while waiting, ignoring")
}
...
func (s *StreamConn) validateSynAckAckThrough(pkt *Packet) error {
	...
	if pkt.AckThrough != expectedAck {
		log.WithFields(map[string]interface{}{
			...
			"received": pkt.AckThrough,
		}).Warn("SYN-ACK AckThrough does not match our SYN sequence")
...
func (s *StreamConn) validateRemoteSequenceNum(pkt *Packet) {
	if pkt.SequenceNum == 0 {
		...
		log.WithField("remoteStreamID", pkt.SendStreamID).Warn("SYN-ACK has sequence number 0; unusual for random ISN but accepting")
	}
}
...
func (s *StreamConn) extractRemoteMTUFromSynAck(pkt *Packet) {
	if pkt.Flags&FlagMaxPacketSizeIncluded != 0 && pkt.MaxPacketSize > 0 {
		...
		log.WithFields(map[string]interface{}{
			"remoteMTU":     s.remoteMTU,
			"localMTU":      s.localMTU,
			"negotiatedMTU": s.getNegotiatedMTULocked(),
		}).Debug("MTU negotiated from SYN-ACK")
	} else {
		s.remoteMTU = DefaultMTU
		log.WithFields(map[string]interface{}{
			"localMTU":      s.localMTU,
			"negotiatedMTU": s.getNegotiatedMTULocked(),
		}).Warn("SYN-ACK missing MTU flag/value, using default")
	}
}
...
func (s *StreamConn) extractRemoteProfileFromSynAck(pkt *Packet) {
	if pkt.Flags&FlagProfileInteractive != 0 {
		...
		log.Debug("remote profile: interactive")
	} else {
		s.remoteProfile = ProfileBulk
		log.Debug("remote profile: bulk")
	}
}
...
func (s *StreamConn) logSynAckProcessed(pkt *Packet) {
	log.WithFields(map[string]interface{}{
		"remoteSeq":      pkt.SequenceNum,
		"ourAck":         s.recvSeq,
		"remoteStreamID": s.remoteStreamID,
		"remoteMTU":      s.remoteMTU,
		"ackThrough":     s.ackThrough,
	}).Debug("processed SYN-ACK")
}
...
func ListenWithManager(manager *StreamManager, localPort uint16, mtu int) (*StreamListener, error) {
	...
	log.WithFields(map[string]interface{}{
		"port": localPort,
		"mtu":  mtu,
	}).Info("listening for connections (with manager)")
...
func (l *StreamListener) Close() error {
	...
	log.WithField("port", l.localPort).Info("closed listener")
...
func (l *StreamListener) validateSYNSource(synPkt *Packet, remoteDest *go_i2cp.Destination) (*go_i2cp.Destination, error) {
	...
	if synPkt.Flags&FlagFromIncluded != 0 && synPkt.FromDestination != nil {
		...
		log.Debug("using FromDestination from SYN packet")
	}

	if peerDest == nil {
		log.Error("SYN packet has no source destination - cannot reply")
...
func (l *StreamListener) verifySYNSignature(synPkt *Packet) error {
	...
	if err := VerifyPacketSignature(synPkt, nil); err != nil {
		log.WithError(err).Warn("SYN signature verification failed - rejecting packet")
		return fmt.Errorf("SYN signature verification failed: %w", err)
	}
	log.Debug("SYN signature verified successfully")
	return nil
}
...
func (l *StreamListener) verifySYNReplayPrevention(synPkt *Packet) error {
	...
	if err != nil {
		log.WithError(err).Warn("failed to hash destination for replay prevention check")
		return nil
	}
	...
	for i := 0; i < 8; i++ {
		...
		if synPkt.NACKs[i] != expected {
			log.Warn("SYN replay prevention check failed - rejecting")
			return fmt.Errorf("SYN replay prevention check failed")
		}
	}
	log.Debug("SYN replay prevention check passed")
	return nil
}
...
func extractRemoteMTU(synPkt *Packet, localMTU uint16) uint16 {
	if synPkt.Flags&FlagMaxPacketSizeIncluded != 0 && synPkt.MaxPacketSize > 0 {
		log.WithFields(map[string]interface{}{
			...
			"localMTU":  localMTU,
		}).Debug("extracted MTU from SYN")
		return synPkt.MaxPacketSize
	}
	log.Warn("SYN missing MTU, using default")
	return DefaultMTU
}
...
func (l *StreamListener) queueConnectionForAccept(conn *StreamConn, remotePort uint16) error {
	...
	case l.acceptChan <- conn:
		log.WithFields(map[string]interface{}{
			"localPort":  l.localPort,
			"remotePort": remotePort,
		}).Debug("queued connection for Accept()")
		return nil
	default:
...
func (s *StreamConn) waitForChokeCondVar() error {
	for s.choked && time.Now().Before(s.chokedUntil) && !s.closed {
		...
		log.WithField("chokedUntil", s.chokedUntil).Debug("peer is choked - waiting for unchoke or timeout")
...
func (s *StreamConn) waitForChokeFallback() error {
	...
	log.WithField("waitDuration", waitDuration).Debug("peer is choked - waiting before sending (fallback)")
...
func (s *StreamConn) waitForWindowSpace(bytesWritten int) error {
	for s.sendCond != nil && s.sentPackets != nil && uint32(len(s.sentPackets)) >= s.cwnd && !s.closed {
		...
		log.WithFields(map[string]interface{}{
			"inFlight": len(s.sentPackets),
			"cwnd":     s.cwnd,
		}).Debug("window full - waiting for ACKs")
...
func (s *StreamConn) consumeBufferDataLocked(buf []byte) (int, error) {
	...
	log.WithField("bytes", n).Debug("read data")
	return n, nil
}
	...
	if s.sendingChoke {
		...
		if bufferUsage < 0.3 {
			if err := s.sendUnchokeSignalLocked(); err != nil {
				log.WithError(err).Warn("failed to send unchoke signal after read")
			}
		}
	}
}
...
func (s *StreamConn) retransmitTimer() {
	log.Debug("retransmit timer started")
	defer log.Debug("retransmit timer stopped")
	...
	for {
		...
		case <-ticker.C:
			if err := s.checkRetransmissions(); err != nil {
				...
				log.WithError(err).Error("retransmission failed, closing connection")
				s.Close()
				return
			}
		}
...
func (s *StreamConn) applyExponentialBackoffLocked(timedOutCount int) {
	...
	log.WithFields(map[string]interface{}{
		...
		"oldRTO": oldRTO,
		"newRTO": s.rto,
	}).Debug("packets timed out, applying exponential backoff")
}
...
func (s *StreamConn) retransmitTimedOutPacketsLocked(timedOutSeqs []uint32) error {
	for _, seq := range timedOutSeqs {
		...
		if err := s.retransmitPacketLocked(seq); err != nil {
			log.WithError(err).WithField("seq", seq).Warn("timeout retransmission failed")
			continue
		}
		...
		log.WithFields(map[string]interface{}{
			...
			"rto":        s.rto,
		}).Debug("retransmitted packet due to timeout")
	}
...
func (s *StreamConn) inactivityTimer() {
	...
	defer log.Debug("inactivity timer stopped")
...
func (s *StreamConn) checkInactivity() {
	...
	}).Debug("inactivity timeout reached - sending keepalive ACK")
	...
	if err := s.sendAckLocked(); err != nil {
		log.WithError(err).Warn("failed to send keepalive ACK")
		return
	}
...
func (s *StreamConn) receiveLoop() {
	...
	defer log.Debug("receive loop stopped")
	...
	for {
		...
		case pkt := <-s.recvChan:
			...
			if err := s.processPacket(pkt); err != nil {
				log.WithError(err).WithField("seq", pkt.SequenceNum).Warn("failed to process packet")
			}
		}
	}
}
...
func (s *StreamConn) dispatchPacketByFlags(pkt *Packet) error {
	...
	default:
		log.WithField("flags", pkt.Flags).Debug("packet with no recognized flags")
	}
	return nil
}
...
func (s *StreamConn) handleSynAckLocked(pkt *Packet) error {
	if s.state != StateSynSent {
		log.WithField("state", s.state.String()).Warn("unexpected SYN-ACK in state")
		return fmt.Errorf("unexpected SYN-ACK")
	}

	log.WithField("remoteSeq", pkt.SequenceNum).Debug("received SYN-ACK")
	...
	if pkt.Flags&FlagMaxPacketSizeIncluded != 0 && pkt.MaxPacketSize > 0 {
		...
		log.WithFields(map[string]interface{}{
			...
			"localMTU":      s.localMTU,
			"negotiatedMTU": s.getNegotiatedMTULocked(),
		}).Debug("MTU negotiated")
	} else {
		s.remoteMTU = DefaultMTU
		log.Warn("SYN-ACK missing MTU, using default")
	}
...
func (s *StreamConn) handleFinalAckLocked(pkt *Packet) error {
	log.WithField("ack", pkt.AckThrough).Debug("received final ACK")

	// Verify ACK is for our SYN-ACK
	if pkt.AckThrough != s.sendSeq-1 {
		log.WithFields(map[string]interface{}{
			"expected": s.sendSeq - 1,
			"got":      pkt.AckThrough,
		}).Warn("ACK number mismatch")
	}
	...
	log.WithFields(map[string]interface{}{
		"state":         s.state.String(),
		"negotiatedMTU": s.getNegotiatedMTULocked(),
	}).Info("server-side connection established")
...
func (s *StreamConn) handleCloseLocked(pkt *Packet) error {
	...
	if pkt.Flags&FlagSignatureIncluded != 0 {
		if err := VerifyPacketSignature(pkt, nil); err != nil {
			log.WithError(err).Warn("CLOSE signature verification failed - rejecting packet")
			return fmt.Errorf("CLOSE signature verification failed: %w", err)
		}
		log.Debug("CLOSE signature verified successfully")
	}
	...
	case StateEstablished:
		...
		if err := s.sendCloseLocked(); err != nil {
			log.WithError(err).Warn("failed to send CLOSE response")
		}
	...
	default:
		log.WithField("state", s.state.String()).Warn("unexpected CLOSE")
	}
...
func (s *StreamConn) handleResetLocked(pkt *Packet) error {
	log.Warn("received RESET - aborting connection")

	// Verify RESET signature if present
	if pkt.Flags&FlagSignatureIncluded != 0 {
		if err := VerifyPacketSignature(pkt, nil); err != nil {
			log.WithError(err).Warn("RESET signature verification failed - rejecting packet")
			return fmt.Errorf("RESET signature verification failed: %w", err)
		}
		log.Debug("RESET signature verified successfully")
	}
...
func (s *StreamConn) calculateRTO() {
	...
	log.WithField("rto", s.rto).Debug("calculated RTO")
}
...
func (s *StreamConn) updateCongestionWindow() {
	if s.cwnd < s.ssthresh {
		...
		log.WithFields(map[string]interface{}{
			...
			"newCwnd":  s.cwnd,
			"ssthresh": s.ssthresh,
		}).Debug("slow start: doubled window")
	} else {
		oldCwnd := s.cwnd
		...
		}).Debug("congestion avoidance: incremented window")
	}
}
...
func (s *StreamConn) handleNACKsLocked(nacks []uint32) {
	...
	log.WithField("nackCount", len(nacks)).Debug("processing NACK requests")
	...
	for _, nackSeq := range nacks {
		...
		log.WithFields(map[string]interface{}{
			"seq":       nackSeq,
			"nackCount": count,
			"threshold": FastRetransmitThreshold,
		}).Debug("received NACK for sequence")
		...
		if count >= FastRetransmitThreshold {
			...
			if err := s.retransmitPacketLocked(nackSeq); err != nil {
				log.WithError(err).WithField("seq", nackSeq).Warn("fast retransmission failed")
			} else {
				log.WithFields(map[string]interface{}{
					"seq":       nackSeq,
					"nackCount": count,
				}).Debug("fast retransmit triggered")
			}
...
func (s *StreamConn) reduceCongestionWindowLocked() {
	...
	log.WithFields(map[string]interface{}{
		"oldCwnd":     oldCwnd,
		"newCwnd":     s.cwnd,
		"oldSsthresh": oldSsthresh,
		"newSsthresh": s.ssthresh,
	}).Debug("congestion detected: reduced window due to packet loss")
}
...
func (s *StreamConn) startPersistTimerLocked() {
	...
	log.WithField("interval", PersistProbeInterval).Debug("persist timer started for choke probing")
...
func (s *StreamConn) stopPersistTimerLocked() {
	if s.persistTimer != nil {
		s.persistTimer.Stop()
		s.persistTimer = nil
		log.Debug("persist timer stopped")
	}
...
func (s *StreamConn) sendProbePacketLocked() {
	...
	data, err := pkt.Marshal()
	if err != nil {
		log.WithError(err).Warn("failed to marshal probe packet")
		return
	}
	...
	err = s.session.SendMessage(s.dest, 6, s.localPort, s.remotePort, stream, 0)
	if err != nil {
		log.WithError(err).Warn("failed to send probe packet")
		return
	}

	log.WithField("seq", pkt.SequenceNum).Debug("sent probe packet while choked")
}
...
func (s *StreamConn) handleOptionalDelayLocked(pkt *Packet) {
	...
	if pkt.OptionalDelay >= MinDelayChoke {
		...
		log.WithFields(map[string]interface{}{
			"delay":       pkt.OptionalDelay,
			"chokedUntil": s.chokedUntil,
		}).Debug("peer is choked - pausing transmission")
		...
		if pkt.OptionalDelay > 0 {
			log.WithField("delay", pkt.OptionalDelay).Debug("peer requests delay")
		}
	}
}
...
func (s *StreamConn) handleAckLocked(pkt *Packet) error {
	...
	if pkt.Flags&FlagNoACK != 0 {
		log.Debug("ignoring ackThrough due to NO_ACK flag")
	} else if seqGreaterThan(pkt.AckThrough, s.ackThrough) {
		oldAck := s.ackThrough
		s.ackThrough = pkt.AckThrough
		log.WithField("ackThrough", s.ackThrough).Debug("updated ackThrough")
...
func (s *StreamConn) handleBufferFullLocked(pkt *Packet, seq uint32) error {
	log.Warn("receive buffer full - buffering packet for later")
	s.outOfOrderPackets[seq] = pkt

	if !s.sendingChoke {
		if err := s.sendChokeSignalLocked(); err != nil {
			log.WithError(err).Warn("failed to send choke signal")
		}
	}

	if err := s.sendAckLocked(); err != nil {
		log.WithError(err).Warn("failed to send ACK during buffer full")
	}
...
func (s *StreamConn) updateChokeStateForUsage(bufferUsage float64) {
	if bufferUsage > 0.8 && !s.sendingChoke {
		...
		if err := s.sendUnchokeSignalLocked(); err != nil {
			log.WithError(err).Warn("failed to send unchoke signal")
		}
	}
}
...
func (s *StreamConn) handleOutOfOrderPacketLocked(pkt *Packet, seq uint32) error {
	log.WithFields(map[string]interface{}{
		"expected": s.recvSeq,
		"got":      seq,
	}).Debug("out-of-order packet - buffering")
	...
	if err := s.sendAckLocked(); err != nil {
		log.WithError(err).Warn("failed to send ACK with NACKs")
	}
...
func (s *StreamConn) handleDataLocked(pkt *Packet) error {
	...
	if seqLessThan(seq, s.recvSeq) {
		log.WithFields(map[string]interface{}{
			"expected": s.recvSeq,
			"got":      seq,
		}).Debug("duplicate or old packet - ignoring")
		return nil
	}
...
func (s *StreamConn) deliverBufferedPacketsLocked() {
	for {
		...
		n, err := s.recvBuf.Write(pkt.Payload)
		if err != nil {
			log.WithError(err).WithField("seq", s.recvSeq).Error("receive buffer full while delivering buffered packet")
		...
		log.WithFields(map[string]interface{}{
			"bytes": n,
			"seq":   s.recvSeq,
		}).Debug("delivered buffered packet")
...
func (s *StreamConn) calculateSequenceGapLocked(receivedSeq uint32) uint32 {
	...
	if gap > maxIterations {
		...
		}).Debug("large sequence gap detected, limiting NACK iteration")
	...
	for i := uint32(0); i < gap; i++ {
		...
		if len(s.nackList) >= MaxNACKs {
			log.WithFields(map[string]interface{}{
				...
				"droppedSeq": seq,
			}).Debug("NACK list at capacity, dropping new entry")
			break
		}
...
func (s *StreamConn) removeFromNACKListLocked(seq uint32) {
	if _, exists := s.nackList[seq]; exists {
		...
		log.WithFields(map[string]interface{}{
			"seq":       seq,
			"remaining": len(s.nackList),
		}).Debug("removed from NACK list")
	}
}
...
func (s *StreamConn) cleanupAckedPacketsLocked(oldAck, newAck uint32, currentNACKs []uint32) {
	...
	if cleaned > 0 || nackCountsCleaned > 0 {
		log.WithFields(map[string]interface{}{
			...
			"newAck":            newAck,
			"cleaned":           cleaned,
			"nackCountsCleaned": nackCountsCleaned,
			"remaining":         len(s.sentPackets),
		}).Debug("cleaned up ACKed packets")
...
func (s *StreamConn) retransmitPacketLocked(seq uint32) error {
	...
	if !exists {
		...
		log.WithField("seq", seq).Debug("packet already ACKed, skipping retransmit")
		return nil
	}
...
func (s *StreamConn) handleIncomingPacket(pkt *Packet) error {
	log.WithFields(map[string]interface{}{
		...
		"flags":   pkt.Flags,
		"payload": len(pkt.Payload),
	}).Debug("queuing packet for receiveLoop")
	...
	case <-s.ctx.Done():
		return fmt.Errorf("connection closed")
	default:
		log.Warn("receive channel full, dropping packet")
...
func (s *StreamConn) sendAckLocked() error {
	...
	if len(s.nackList) > 0 {
		...
		log.WithField("nackCount", len(pkt.NACKs)).Debug("including NACKs in ACK packet")
	}
...
func (s *StreamConn) sendChokeSignalLocked() error {
	...
	log.WithField("bufferUsage", float64(s.recvBuf.TotalWritten())/float64(s.recvBuf.Size())).Debug("sending choke signal to peer")
...
func (s *StreamConn) sendUnchokeSignalLocked() error {
	...
	log.WithField("bufferUsage", float64(s.recvBuf.TotalWritten())/float64(s.recvBuf.Size())).Debug("sending unchoke signal to peer")
...
func (s *StreamConn) trackPacketForRetransmission(pkt *Packet, data []byte) {
	...
	log.WithFields(map[string]interface{}{
		"seq":     pkt.SequenceNum,
		"tracked": len(s.sentPackets),
	}).Debug("tracking sent packet for retransmission")
}
...
func (s *StreamConn) sendCloseIfEstablished() {
	...
	if err := s.sendCloseLocked(); err != nil {
		log.WithError(err).Warn("failed to send CLOSE packet")
	}
...
func (s *StreamConn) sendCloseLocked() error {
	...
	if s.session != nil {
		...
		keyPair, err := s.session.SigningKeyPair()
		if err != nil {
			log.WithError(err).Warn("failed to get signing key pair for CLOSE packet")
			// Continue anyway - signing is best effort for CLOSE
		} else if err := SignPacket(pkt, keyPair); err != nil {
			log.WithError(err).Warn("failed to sign CLOSE packet")
			// Continue anyway - signing is best effort for CLOSE
		}
	}
...
func (s *StreamConn) handleMessageDelivered(seqNum uint32) {
	...
	if s.sentPackets != nil {
		if _, exists := s.sentPackets[seqNum]; exists {
			...
			log.WithField("seq", seqNum).Debug("message confirmed delivered by I2CP")
		}
	}
...
func (s *StreamConn) handleMessageFailed(seqNum uint32, retriable bool) {
	...
	log.WithFields(map[string]interface{}{
		"seq":       seqNum,
		"retriable": retriable,
	}).Warn("message delivery failed at I2CP layer")
	...
	if s.sentPackets != nil {
		if pktInfo, exists := s.sentPackets[seqNum]; exists {
			...
			if pktInfo.retryCount <= MaxRetransmissions {
				log.WithFields(map[string]interface{}{
					"seq":        seqNum,
					"retryCount": pktInfo.retryCount,
				}).Info("retransmitting packet after I2CP failure")

				// Re-send the stored packet data
				if err := s.sendViaI2CP(pktInfo.data); err != nil {
					log.WithError(err).WithField("seq", seqNum).Warn("retransmission failed")
				}
			} else {
				log.WithFields(map[string]interface{}{
					"seq":        seqNum,
					"retryCount": pktInfo.retryCount,
				}).Warn("max retransmissions exceeded for I2CP failure")
			}
		}
	}
}
